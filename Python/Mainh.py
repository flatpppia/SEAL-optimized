...
parser.add_argument('--d-name', default='cora', help='network name')
...
parser.add_argument('--max-train-num', type=int, default=20480,
                    help='set maximum number of train links (to fit into memory)')
parser.add_argument('--max-test-num', type=int, default=2048,
...
parser.add_argument('--max-nodes-per-hop', type=int, default=64, help='if > 0, upper bound the nodes per hop by subsampling')
...
if args.d_name is not None:  # use .mat network
    if args.d_name == 'syn-cora':
        ...
        def setlabels(label_list):
            ...
            return L
        args.d_name = 'ind.' + args.d_name
        dataset = PlanetoidData(dataset_str=args.d_name, dataset_path='data/data')
        ...
        net = adj.tocsc(adj)
    elif args.d_name=='actor' or args.d_name=='chameleon':  # actor:node7600 ;chameleon: node2277,link31371
        args.d_name = args.d_name + '.npz'
        G = load_npzdata(args.d_name)
        adj = G['adj']
...
if args.train_name is None and args.test_name is None:
    ...
    train_pos, train_neg, test_pos, test_neg = sample_neg(net, train_pos=train_pos, test_pos=test_pos,
                            max_train_num=args.max_train_num, all_unknown_as_negative=args.all_unknown_as_negative)
...
if args.only_predict:  # no need to use negatives
    ...
    _, test_graphs, max_n_label = links2subgraphs(A, None, None, test_pos, None, single_labels, args.hop,
    ...
    train_graphs, test_graphs, max_n_label = links2subgraphs(A, train_pos, train_neg, test_pos, test_neg,
                                single_labels, args.n_paral, args.hop, args.max_nodes_per_hop, node_information, args.no_parallel)
...
with open("results_txt/" + args.d_name + "_sortnewl_result_k" + str(int(args.sortpooling_k*10)) + ".txt", "a+") as ff:
