...
from HeterophilicGraph import normalize_matrix, construct_heterophilic_graph
...
def H_final(A, X):
    ...
    for i in range(X.shape[0]):
        a = X_norm[i].reshape([1, X.shape[1]])
        for j in range(i + 1, X.shape[0]):
            b = X_norm[j].reshape([1, X.shape[1]])
            W[i][j] = W[j][i] = torch.mm(a, b.t())
    # print(np.count_nonzero(W))
    # time.sleep(100)
    I = np.eye(W.shape[0])
    W = W.numpy() + I
    if X.shape[0] < 5:
        H = construct_heterophilic_graph(K=W, A=A, Knn=X.shape[0])
    else:
        H = construct_heterophilic_graph(K=W, A=A)
    # 构造实对称矩阵
    H1 = np.transpose(H)
    H = H + H1
    index = np.where(H == 2)
    for k in range(len(index[0])):
        H[index[0][k]][index[1][k]] = H[index[1][k]][index[0][k]] = 1

    return H

#* this is core code for construct S, part 6
def A_final(S, nbs=400.0):
    S[S < (1 / nbs)] = 0
    S[S >= (1 / nbs)] = 1
    S[S > 0] = 1
    S = S.T + S
...
if __name__ == '__main__':
    ...
    X, A, gnd = Swither[2](dataname="Cora")  # numpy.ndarray  X:(2708, 1433) gnd:(2708,)

    # time.sleep(100)
    H = H_final(A, torch.tensor(X, dtype=torch.float))
    print(type(H))  # numpy.ndarray
    ...
    S = op_S(lb2, N, M)
    S = A_final(S)
    print("homophily A:", homophily_v2(A, gnd))  #! 0.8137
    print("homophily H:", homophily_v2(H, gnd))  #!
    print("homophily S:", homophily_v2(S, gnd))  #! 0.8691 higher than the reported result
    print("edges of A:", np.count_nonzero(A))  #! 5429
    print("edges of H:", np.count_nonzero(H))  #!
